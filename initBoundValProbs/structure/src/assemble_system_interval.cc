/*
*IGA_structure<dim>::assemble_system_interval
*this function assembles jacobian and righ hand side 
*use "structureModel" defined in "run"
...
template <int dim>
void IGA_structure<dim>::assemble_system_interval (const typename std::vector<knotSpan<dim> >::iterator &begin, const typename std::vector<knotSpan<dim> >::iterator &end){
  //element loop
  ...
  for (typename std::vector<knotSpan<dim> >::iterator cell=begin; cell<end; cell++){
    ...
    for(unsigned int i=0; i<dofs_per_cell; ++i) R[i]=0.0; 
		//dislocation
		structureModel->getResidual(*cell, fe_values, ULocal, R, IGA<dim>::currentIteration);
